	...
	export default {
		...
		watch: {
			current: {
				immediate: true,
				handler(nVal) {
					console.log("sY1->",nVal,this.list)
			...
			list: {
				immediate: true,
				handler(nVal) {
					console.log("sY2->",nVal,this.list)
					this.calculate()
				}
			}
		},
		...
		methods: {
			calculate(){
				this.listInfo = this.list.map((val, index) => {
					if (typeof val == 'object') {
						let obj = {
							width: (this.width-6)/this.list.length,
							name: val.label,
							key:val.value
						};
						return obj;
			...
			click(index) {
				...
				this.$emit('change', this.listInfo[index].key,this.name);
