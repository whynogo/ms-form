					...
					<view
					 ...
					 :style="{
						 ...
						 position: hIndex === 0 && firstLineFixed ? 'sticky' : 'static'}">
						 		<u-image v-if="isShowImage(header)" width="100%" height="100%" :src="autoContentItem(cIndex, hIndex)"></u-image>
								<span v-if="!isShowImage(header)">{{autoContentItem(cIndex, hIndex)}}</span>
						 </view>
				</view>
...
<script>
	...
	export default {
		data() {
			return {
				src:"https://miao-share.oss-cn-shenzhen.aliyuncs.com/sys_images/area0.png",
				bottomComputed: [],
				colorList: [],
				bgColorList: [],
		...
		computed: {
			...
			autoContentItem() {
				return function(cIndex, hIndex) {
					...
					if (content[header.key] || content[header.key] === 0) {
						if(typeof this.options[header.key]=="undefined"){
							result = content[header.key]
							var formatter=this.formatters[header.key]
							if(typeof formatter!="undefined"){
								result=formatter(result,cIndex, hIndex)
							}
						}else{
							var value = content[header.key]
							var item= this.options[header.key].find(s=> {return s.value===value})
							result=("undefined" != typeof item)?item.label:(value+" in "+header.key+"在option中未定义")
						}
			...
			autoContentBgColor() {
				return function(cIndex, hIndex) {
					...
					var formatter=this.formatters["background"]
					if(typeof formatter!="undefined"){
						result=formatter(content,cIndex, hIndex)
					}
					return result
				}
			},
			autoContentColor() {
				return function(cIndex, hIndex) {
					...
					var formatter=this.formatters["foreground"]
					if(typeof formatter!="undefined"){
						result=formatter(content,cIndex, hIndex)
					}
					return result
		...
		methods: {
			...
			isShowCol(colIndex){
				...
				return !('hidden' in this.headers.find(it=>{return it.order==colIndex}))
			},
