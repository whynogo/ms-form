<template>
	<view class="z-table" @click="$emit('tableClick')" @longpress="$emit('tableLongpress')" @longtap="$emit('tableLongpress')">
				...
				<view class="z-table-pack">
					<view class="z-table-title">
						<view class="z-table-title-item" :class="{ 'z-table-stick-side': stickSide && index == 0 }" :style="{ width: item.width ? item.width + 'rpx' : '200rpx' }"
						 v-for="(item, index) in columns" :key="index" @click="sort(item.key, index)" v-if="!item.hasOwnProperty('hidden')">
							<view v-if="showSelect && !singleSelect && isFirstVisibleCol(index)" class="select-box" @click="doSelect(true)">
							...
							<view :class="['z-table-col-text', {'text-left': titleTextAlign === 'left', 'text-center': titleTextAlign === 'center', 'text-right': titleTextAlign === 'right'}]">
								...
								<view v-if="item.hasOwnProperty('key') && item.hasOwnProperty('sort') && tableData.length" class="sort">
									<view class="up-arrow" :class="{ action: sortState(item,'asc') }"></view>
									<view class="down-arrow" :class="{ action: sortState(item,'desc') }"></view>
								</view>
							</view>
						</view>
					</view>
					<view v-if="tableData.length" :class="['table-container-box', {'short-table': !longTable && showBottomSum}]">
						<view class="z-table-container-row" :class="{ 'z-table-has-bottom': showBottomSum }" v-for="(row, iIndex) in tableData"
						 :key="iIndex">
							<view :class="['z-table-container-col', { 'z-table-stick-side': stickSide && jIndex == 0 }]" :style="{backgroundColor:getBgColor(iIndex,jIndex), width: col.width ? col.width + 'rpx' : '200rpx' }"
							 v-for="(col, jIndex) in columns" :key="jIndex" @click="itemClick(row, col)" v-if="!col.hasOwnProperty('hidden')">
								<view v-if="showSelect && isFirstVisibleCol(jIndex)" class="select-box" @click="doSelect(false, iIndex)">
								...
								<view :class="['z-table-col-text', {'text-left': textAlign === 'left', 'text-center': textAlign === 'center', 'text-right': textAlign === 'right'}]">
									...
									<!-- #endif -->
									<u-button v-if="'action'==col.type" type="primary" size="mini" @click="doAction(row,iIndex)">操作</u-button>
...
//因为改变sort后重新获取数据,会导致sort状态复位,只好这样改为全局变量	
var g_nowSortKey= ''
var g_sortType='desc' // asc/desc 升序/降序
	...
	export default {
		...
		methods: {
			...
			getRowContent(row, col) {
				...
				if(col.key in this.options){
					var item= this.options[col.key].find(s=> {return s.value===rowKey})
					tempHTML=("undefined" != typeof item)?item.label:(value+" in "+rowKey+"在option中未定义")
					if("state" in item)
						tempHTML=`<span class='state-text ${item.state}'>&ensp;${tempHTML}&ensp; </span>`
				}
				if("date"==col.type){
					tempHTML=moment(rowKey).format("YYYY-MM-DD HH:mm")
				}else if("time"==col.type){
					tempHTML=moment("2021-02-01 "+rowKey).format("HH:mm")
				}else if("action"==col.type){
//					tempHTML="<button size='mini' onclick=\"var msg='showAction';window.parent.postMessage(msg, '*')\">操作</button>"
					tempHTML=""
				}
			...
			sort(key, index) {
				...
				console.log("sort",key,g_nowSortKey,g_sortType)
				if (key != g_nowSortKey) {
					g_nowSortKey = key
					g_sortType = 'desc'
				} else {
				...
				console.log("sort=>",key,this.nowSortKey,this.sortType)
			},
			toggleSort() {
				g_sortType = g_sortType == 'asc' ? 'desc' : 'asc'
			},
			numTransform(n) {
				if (Number.isNaN(n - 0)) {
					return n
			...
			sortState(item,type){
				return g_nowSortKey == item.key && g_sortType == type
			},
			setUrl(row, col) {
				if (!col.isLink) {
					return
			...
			doAction(row,iIndex){
				console.log("doAction=>",row,iIndex)
				if(!this.selectArr.includes(iIndex)){
					this.selectArr.push(iIndex)
					this.$forceUpdate()
					var ids=this.selectArr.map(it=>{
						return this.tableData[it].id
					})
					this.$emit('onSelect', ids)
				}	
			...
			isFirstVisibleCol(colIndex){
				for(var i=0;i<this.columns.length;i++)
				  if(!this.columns[i].hasOwnProperty("hidden"))
				      return i==colIndex
				return false	  
			},
			isSelected(row){
				return this.selectArr.find(it=>{return it==row})
			},
			getBgColor(row,col){
