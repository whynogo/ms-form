...
<script>
import Emitter from '@/node_modules/uview-ui/libs/util/emitter.js';
import {
	isEmpty,
} from '@/common/util.js';
/**
...
export default {
	...
	watch: {
		current: {
			immediate: true,
			handler(nVal, oVal) {
				// /console.log("Y=>",nVal,oVal)
				this.defaultValue = nVal;
			}
		},
		value(nVal, oVal) {
			//console.log("YY=>",nVal,oVal)
			this.defaultValue = nVal;
	...
	methods: {
		...
		handleInput(event) {
			...
			// 判断是否去除空格
			if(this.trim&&!isEmpty(event.detail.value)) value = this.$u.trim(value);
			// vue 原生的方法 return 出去
			...
			setTimeout(() => {
				...
				this.$emit("textChanged",this.defaultValue,this.name)//这里小程序要与H5分开
			}, 40)
