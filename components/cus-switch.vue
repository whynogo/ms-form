<template>
	<view class="u-switch" :class="[defaultValue == true ? 'u-switch--on' : '', disabled ? 'u-switch--disabled' : '']" @tap="onClick"
	 :style="[switchStyle]">
	...
	export default {
		...
		computed: {
			switchStyle() {
				...
				style.backgroundColor = this.defaultValue ? this.activeColor : this.inactiveColor;
				return style;
			},
			loadingColor() {
				return this.defaultValue ? this.activeColor : null;
			}
		},
		methods: {
			onClick() {
				if (!this.disabled && !this.loading) {
					...
					//console.log("switch=>",this.defaultValue)
					this.$emit('change', this.defaultValue ,this.name);
				}
			}
		}
	};
